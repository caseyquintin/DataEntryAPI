    ...
    return Promise.all([
        ...
        $.getJSON('http://localhost:5062/api/options/shipline').then(data => {
            const cleaned = data
                ...
                .filter(name => !!name); // remove null/undefined/empty strings
        
            const uniqueSorted = [...new Set(cleaned)].sort();
        
            // ⬆️ Move "UNKNOWN" to the top if it exists
            const unknown = uniqueSorted.filter(x => x === 'UNKNOWN');
            const rest = uniqueSorted.filter(x => x !== 'UNKNOWN');
        
            shiplineOptions = [...unknown, ...rest];
        }),
        $.getJSON('http://localhost:5062/api/vessels/vessel-lines').then(data => {
            vesselLineOptions = data;
            console.log("🚢 Vessel Lines:", vesselLineOptions);
        }),
...
function initializeDataTableAndHandlers () {
        ...
        const table = $('#ContainerList').DataTable({
            ...
            columns: [
                ...
                {
                    data: 'vesselID',
                    visible: false // hidden, backend only
                },
                {
                    data: 'vesselLine',
                    className: 'editable'
            ...
            rowCallback: function(row, data) {
                ...
                $('#ContainerList tbody').on('click', 'td.editable', async function () {
                    ...
                    if (fieldName === 'currentStatus') {
                        ...
                        isDropdownField = ['Actual', 'Estimate'];
                        console.log(`🎯 Detected special dropdown for ${fieldName}:`, isDropdownField);
                    } else if (fieldName === 'mainSource') {
                        isDropdownField = ['Shipline', 'Ship Owner', 'Terminal', 'Railway', 'Other'];
                        console.log(`🎯 Detected special dropdown for ${fieldName}:`, isDropdownField);
                    } else if (['rail', 'transload'].includes(fieldName)) {
                        isDropdownField = ['Yes', 'No'];
                        console.log(`🎯 Detected special dropdown for ${fieldName}:`, isDropdownField);
                    ...
                    } else if (fieldName.toLowerCase() === 'vesselline') {
                        isDropdownField = vesselLineOptions;
                    } else if (fieldName.toLowerCase() === 'vesselname') {
                        const vesselLineID = rowData.vesselLineID || rowData.shiplineID;
                        if (!vesselLineID) {
                            console.warn("⚠️ No vesselLineID found — skipping Vessel Name dropdown");
                            return;
                        }
                    
                        try {
                            const vesselData = await $.getJSON(`http://localhost:5062/api/vessels/by-line/${vesselLineID}`);
                            console.log("🚢 Vessel data:", vesselData);
                    
                            // Dropdown with vessel name → ID mapping
                            isDropdownField = vesselData.map(v => ({
                                value: v.vesselID,
                                label: v.vesselName
                            }));
                        } catch (err) {
                            console.error("❌ Failed to load vessels:", err);
                            showToast("Failed to load vessel list", "danger");
                        }
                    }
                    ...
                    input.on('change', async function () {
                        ...
                        const match = shiplineOptions.find(opt => opt === selectedLine);
                        console.log("🚢 Vessel line selected:", selectedLine);
                        ...
                        try {
                            const vesselLineID = rowData.vesselLineID || rowData.shiplineID;
                            if (!vesselLineID) {
                                console.warn("⚠️ No vesselLineID found on row:", rowData);
                                return;
                            }
                
                            const vesselData = await $.getJSON(`http://localhost:5062/api/vessels/by-line/${vesselLineID}`);
                            console.log("🚢 Vessel data loaded:", vesselData.length);
                
                            // Prepare dropdown
                            const $vesselSelect = $('<select class="form-select form-select-sm"></select>');
                            vesselData.forEach(v => {
                                $vesselSelect.append(`<option value="${v.vesselID}">${v.vesselName}</option>`);
                            });
                
                            const $targetCell = $(table.cell(rowIdx, vesselNameColIdx).node());
                            $targetCell.html($vesselSelect);
                
                            // Trigger save on vessel selection
                            $vesselSelect.on('change', function () {
                                const vesselID = $(this).val();
                                const vesselName = $(this).find('option:selected').text();
                
                                // Save visible name
                                table.cell(rowIdx, vesselNameColIdx).data(vesselName).draw(false);
                                table.cell(rowIdx, vesselIDColIdx).data(Number(vesselID)).draw(false);
                
                                // API patch both values
                                Promise.all([
                                    ...
                                    console.log(`✅ Vessel info saved for ${rowID}: ${vesselName} (${vesselID})`);
                                    $targetCell.removeClass('editing');
                                    moveToNextEditable($targetCell[0]);
                                }).catch(err => {
                                    console.error("❌ Failed to save vessel info", err);
                                    showToast("Failed to update vessel fields.", "danger");
                                });
                            });
                
                            $vesselSelect.trigger('focus');
                        } catch (err) {
                            console.error("❌ Failed to fetch vessels for line", err);
                            showToast("❌ Could not load vessel list.", "danger");
                        }
                    });

                    if (fieldName.toLowerCase() === 'vesselname') {
                    ...
                    if (fieldName.toLowerCase() === 'terminal') {
                        input.on('change', function () {
                            ...
                            $(cell.node()).removeClass('editing');
                            setTimeout(() => moveToNextEditable(cell.node()), 0);
                            $('#ContainerList td.editing').removeClass('editing');
                        });
                    }
                    ...
                    if (isDateField) {
                        flatpickr(input[0], {
                            ...
                            onChange: function(selectedDates, dateStr, instance) {
                                input.val(dateStr).trigger('input').trigger('change');
                                console.log("📅 Date selected via picker:", dateStr);
                    
                                // Optional: auto-tab after selecting a date
                                setTimeout(() => {
                    ...
                    input.on('keydown', function (e) {
                        console.log("🎯 Keydown caught:", e.key);
                        if (e.key === 'Tab' || e.key === 'Enter') {
                            console.log("⏩ Tab or Enter pressed");
                            e.preventDefault();
                            preventBlur = true;
                            console.log("✅ preventBlur set to true, moving to next editable field");
                            ...
                            const terminalOptions = isDropdownField ?? [];
                            ...
                            if (isTerminalField && Array.isArray(terminalOptions)) {
                                ...
                                $(cell.node()).removeClass('editing');
                                setTimeout(() => {
                                    console.log("🧭 Calling moveToNextEditable (terminal) with:", cell.node());
                                    moveToNextEditable(cell.node());
                                }, 0);
                                return;
                            }

                            if (fieldName.toLowerCase() === 'vesselname' && Array.isArray(isDropdownField)) {
                                const match = isDropdownField.find(opt => opt.value == newValue);
                                const vesselName = match?.label ?? '[Unknown Vessel]';
                            
                                // ✅ Update name
                                cell.data(vesselName).draw(false);
                            
                                // ✅ Update hidden vessel ID
                                ...
                                // ✅ PATCH both
                                const patchVesselName = fetch('http://localhost:5062/api/containers/update-field', {
                                    method: 'PATCH',
                                    headers: { 'Content-Type': 'application/json' },
                                    body: JSON.stringify({
                                        containerID: rowID,
                                ...
                                const patchVesselID = fetch('http://localhost:5062/api/containers/update-field', {
                                    method: 'PATCH',
                                    headers: { 'Content-Type': 'application/json' },
                                    body: JSON.stringify({
                                        containerID: rowID,
                                ...
                                Promise.all([patchVesselName, patchVesselID])
                                    .then(() => {
                                        console.log("✅ Vessel info saved");
                                        $(cell.node()).removeClass('editing');
                                        moveToNextEditable(cell.node());
                                    })
                                    .catch(err => {
                                        console.error("❌ Error saving vessel info:", err);
                                        showToast("Failed to update vessel", "danger");
                                    });
                            
                                return;
                            }

                            // 🛑 Only skip fallback if this was a terminal dropdown edit
                            if (isTerminalField && isDropdownField) {
                                console.warn("⚠️ Skipping fallback save — terminal already handled.");
                                return;
                            }

                            // 🔁 Re-evaluate change just before using it again (in case original was shadowed)
                            const changed = newValue !== originalValue; 
 
                            console.log("🧪 Edit change check:", {
                                originalValue,
                                newValue,
                            ...
                            if (changed && newValue !== '') {
                                console.log("✅ Entered fallback save block");
                                ...
                                console.warn("⛔ Skipping fallback save (changed?", changed, "| empty?", newValue === '', ")");

                                // Cleanup UI before next
                                cell.data(originalValue); // No draw needed
                                $(cell.node()).removeClass('editing');
                                
                                // Move on immediately
                                requestAnimationFrame(() => {
                                    console.log("🧭 [Fast path] moveToNextEditable");
                                    moveToNextEditable(cell.node());
                                });
                            }
                            
                            // 🧭 Always attempt to move focus, regardless of change
                            setTimeout(() => {
                                console.log("🧭 [Always called] moveToNextEditable");
                                moveToNextEditable(cell.node());
                            }, 0);
                        }
        ...
        $(document).on('click', '#bulkDeleteBtn', function () {
            ...
            // Save IDs for the actual confirm button handler
            pendingBulkDeleteIDs = selectedIDs;
        
            // Fetch and show container numbers
            const $list = $('#containerListToDelete').empty();
            selectedIDs.forEach(id => {
                const row = $('#ContainerList').DataTable().row('#' + id).data();
                const num = row?.containerNumber || `[ID ${id}]`;
                $list.append(`<li class="list-group-item">${num}</li>`);
            });
        
            // Show the modal
            new bootstrap.Modal(document.getElementById('confirmBulkDeleteModal')).show();
        });
        ...
        function moveToNextEditable(currentCell) {
            console.log("🔎 moveToNextEditable() called");
            ...
            console.log("➡️ Next cell exists?", $next.length ? "✅ Yes" : "❌ No");
        
            if ($next.length) {
                setTimeout(() => {
                    // 🧹 Clear highlight from the one we're leaving
                    $(currentCell).removeClass('editing');
                    console.log("🧹 Removed `.editing` class from current cell");
        
                    // 🚀 Fire the next editable click
                    console.log("🚀 Triggering click on next editable cell...");
                    $next.trigger('click');
        
                    // 🖌️ Apply highlight to the new one
                    $next.addClass('editing');
                    console.log("🖌️ Applied `.editing` class to next cell");
        
                    setTimeout(() => {
                        const input = $next.find('input, select')[0];
                        if (input) {
                            input.focus();
